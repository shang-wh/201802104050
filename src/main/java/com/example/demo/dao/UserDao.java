...
public final class UserDao {
	...
	public User findByUsername(String username) throws SQLException {
		...
		ResultSet resultSet = preparedStatement.executeQuery();
		//若结果集有一条记录，则以当前记录中的id,description,no,remarks值为参数，创建User对象
		//若结果集中没有记录，则本方法返回null
		if (resultSet.next()){
			user = new User(resultSet.getInt("id"),
					resultSet.getString("username"),
	...
	public boolean update(User user)throws SQLException{
		...
		System.out.println("修改"+affectedRows+"次密码");
	...
	public boolean add(User user) throws SQLException{
		...
		preparedStatement.setString(1,user.getTeacher().getNo());
		preparedStatement.setString(2,user.getTeacher().getNo());
		preparedStatement.setInt(3,user.getTeacher().getId());
		int affectedRows = preparedStatement.executeUpdate();
		System.out.println("增加了"+affectedRows+"个用户");
		preparedStatement.close();
		connection.close();
		return affectedRows>0;
	}
